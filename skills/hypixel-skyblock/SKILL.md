...
- Money routes: bootstrap context, call `skyblock_plan_goal`, and preserve its ranked Bazaar/manual-market, Garden, Dungeon, and Kuudra route candidates with requirements, missing unlocks, output class, budget/cost state, provider freshness, and uncertainty. Do not claim exact profit rates without maintained current sources.
- Farming/Garden routes: use `skyblock_plan_goal` for crop milestone and Jacob contest route candidates before giving manual crop advice; carry missing Garden data and unsupported exact threshold warnings forward.
- Damage, Slayer, Dungeon, or Kuudra goals: bootstrap context, call target-aware `skyblock_readiness` such as `slayer:zombie:t4`, `slayer:eman:t4`, `dungeons:f7`, or `kuudra:burning`, then use its gear/pet/accessory/modifier blockers, budget, prices, and provider freshness before recommending purchases.
...
- Budgeted upgrade/source routes: use `skyblock_plan_goal` so priced buy candidates and unpriced source candidates stay visible before persisting buy, source, or snipe objectives.
...
- Use `skyblock_networth` for sectioned purse, bank, and item networth with unknown prices, price provider freshness, metadata provider freshness, modifier uncertainty, assumptions, and confidence. Use bounded `maxItems`, `timeoutMs`, and `includeItems: false` for compact agent context; use `skyblock_item_networth` when only one section such as armor, equipment, wardrobe, backpacks, accessory bag, or pets is needed.
- Use `skyblock_accessories`, `skyblock_missing_accessories`, and `skyblock_accessory_upgrades` for accessory bag state, duplicate/ignored accessories, recombobulation/enrichment signals, estimated Magical Power, family confidence, missing accessories, and budget-constrained coin-per-MP rankings. Bound broad price sweeps with `maxPriceLookups` and `timeoutMs`.
...
- Use `skyblock_readiness` for heuristic readiness estimates for `dungeons`, `slayer`, `kuudra`, `garden`, or `mining`; pass target suffixes for concrete combat goals and carry forward its target, gear context, failed blockers, assumptions, freshness, and missing-data warnings.
- Use `skyblock_plan_goal` when the user asks what to do next for a concrete goal; pass `useContext: true` for broad planning and preserve route candidates, immediate actions, todo candidates, buy-list candidates, source-item candidates, snipe targets, skip guidance, source freshness, uncertainty, and warnings. Use `persistObjectives: true` only after the user explicitly wants the plan written to durable objectives.
